...
az appconfig kv list --connection-string "$APP_CONFIG_CONNECTION_STRING" --query '[].{key:key, value:value}' -o json > /shared/test1.txt
...
echo "$CONFIG_VALUES" > /shared/test2.txt
